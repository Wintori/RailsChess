class PiecesController < ApplicationController
  ...
  def update
    ...
    if params[:piece][:type] == "Queen" || params[:piece][:type] == "Bishop" || params[:piece][:type] == "Knight" || params[:piece][:type] == "Rook"
      @piece.update(type: params[:piece][:type])
      ...
      @piece.update(piece_params.merge(move_number: @piece.move_number + 1))
    end
  ...
  def switch_turns
    if @game.white_player_user_id == @game.turn_user_id
      @game.update(turn_user_id:@game.black_player_user_id)
    elsif @game.black_player_user_id == @game.turn_user_id
      @game.update(turn_user_id:@game.white_player_user_id)
    end
  end
